...
| 2048     | `FLAG_RUN_LENGTH_ROW_ID_SEGMENTS` | Yes           | Yes             | Row ID sequences may contain `range_with_runs` segments (see [Range with Runs](row_id_lineage.md#range-with-runs-clustered-deletions)). An implementation without this flag decodes such a segment as an unset `oneof` and would report the fragment's row IDs as missing; a writer would drop them when re-encoding, so both must refuse the dataset. Writers emit the encoding only when the table has opted in. |
