...
impl HoleRuns {
    ...
    #[cfg(test)]
    pub fn ends(&self) -> impl Iterator<Item = u32> + '_ {
        (0..self.starts.len()).map(|k| self.end(k))
    }
...
mod tests {
    ...
    fn test_counts_and_ranges() {
        ...
        assert_eq!(runs.ends().collect::<Vec<_>>(), vec![7, 15]);
        assert_eq!(
    ...
    fn test_from_bitmap_matches_cleared_bits() {
        for (len, cleared) in [
            ...
            for (end, &next_start) in runs.ends().zip(runs.starts().iter().skip(1)) {
