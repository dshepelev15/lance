...
pub enum U64Segment {
    ...
    /// support (see `collapse_range_runs` in `serde`). In memory it is two
...
impl U64Segment {
    ...
    /// bitmap when there is less than one range per 128 slots and a hole
    /// array when there is less than one range per four holes.
    pub fn as_ranges(&self) -> Option<Self> {
        ...
        match self {
            ...
            Self::RangeWithHoles { range, holes } => {
                ...
                ((runs.num_runs() + 1) * RANGE_BYTES < holes.len() * 4).then(|| Self::Ranges {
                    range: range.clone(),
                    runs,
                })
            }
...
mod test {
    ...
    fn test_as_ranges_only_when_smaller() {
        ...
        let runs = clustered.as_ranges().unwrap();
        assert!(matches!(runs, U64Segment::Ranges { .. }));
        assert_eq!(
        ...
        assert_eq!(scattered.as_ranges(), None);
    }
}
