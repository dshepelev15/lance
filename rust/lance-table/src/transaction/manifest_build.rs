...
/// `update_config`.
///
/// Manifests written afterwards re-encode bitmap and hole-array row id
/// segments as runs of `Range` segments where that is smaller. That is the
...
fn apply_range_segments(
    ...
    let previously_enabled = current_manifest.is_some_and(range_segments_enabled);
